...
import { canReadSale, SaleActor } from './sale-actor.js';
...
export const MAX_SALES_LIMIT = 500;

// Lo que se puede pedir del histórico de ventas. `from`/`to` son fechas de la venta (cuando se cobró; si
// es un borrador, cuando se creó).
...
export class SaleService {
  ...
  // creó). Para el histórico de ventas: sin sales.view_all (SaleActor.canViewAll) cada quien ve solo
  // las suyas, sea porque las cobró o porque las vendió; con él, ve todas y puede además filtrar por un
  // cajero concreto (`cashierId`). Va acotado: `from`/`to` por fecha y `limit` (por defecto 200, máximo
  // 500) con `offset`, así el histórico no baja años de ventas de una vez. Cada venta trae ya su número
  // de líneas (un solo query, no uno por venta).
  async findAll(
    companyId: string,
    actor: SaleActor,
    filters: SaleFilters = {},
  ): Promise<Sale[]> {
    const { status, storeId, cashierId, from, to } = filters;
    const limit = Math.min(Math.max(filters.limit ?? DEFAULT_SALES_LIMIT, 1), MAX_SALES_LIMIT);
    const offset = Math.max(filters.offset ?? 0, 0);
    const saleDate = 'COALESCE(sale.completedAt, sale.createdAt)';

    const query = this.saleRepository
      .createQueryBuilder('sale')
      .where('sale.companyId = :companyId', { companyId });
    if (status) query.andWhere('sale.status = :status', { status });
    if (storeId) query.andWhere('sale.storeId = :storeId', { storeId });
    if (actor.canViewAll) {
      if (cashierId) query.andWhere('sale.cashierId = :cashierId', { cashierId });
    } else {
      query.andWhere('(sale.cashierId = :me OR sale.sellerId = :me)', { me: actor.userId });
    }
    if (from) query.andWhere(`${saleDate} >= :from`, { from });
    if (to) query.andWhere(`${saleDate} <= :to`, { to });

    const { entities, raw } = await query
      .addSelect('(SELECT COUNT(*)::int FROM "sale_items" si WHERE si."saleId" = sale."id")', 'itemCount')
      .orderBy(saleDate, 'DESC')
      .addOrderBy('sale.id', 'DESC')
      .limit(limit)
      .offset(offset)
      .getRawAndEntities<{ itemCount: number | string }>();

    // Sin uniones, las filas crudas y las entidades salen en el mismo orden.
    entities.forEach((sale, index) => {
      Object.assign(sale, { itemCount: Number(raw[index]?.itemCount ?? 0) });
    });
    return entities;
  }

  // Sin comprobar quién pregunta: para los demás servicios, que ya tienen su propia regla (bloquear la
  // venta, leerla al cobrar). Lo que se le ofrece a un usuario pasa por `findVisible`.
  ...
  // Una venta que quien pregunta puede leer: la suya (la cobró o la vendió), o cualquiera si ve todo o
  // aprueba descuentos o devoluciones. Una que no puede ver se responde como si no existiera: no se le
  // dice ni que está.
  async findVisible(companyId: string, actor: SaleActor, id: string): Promise<Sale> {
    const sale = await this.findOne(companyId, id);
    if (!canReadSale(actor, sale)) throw new NotFoundException(`Venta ${id} no encontrada`);
    return sale;
  }
  ...
  // En el orden en que se agregaron. La empresa y quién puede verlas se comprueban a través de la venta.
  ...
  // solo al cobrarla): así el turno la ve entre sus borradores desde el principio. El borrador NO tiene
  // número: el consecutivo VTA-000123 se asigna al cobrar (`assignNumber`), así los borradores que se
  // abandonan o se borran al cerrar el turno no dejan huecos en la numeración. Con `idempotencyKey`,
  // repetir la misma petición devuelve la venta ya creada en vez de abrir otro borrador.
  async create(
    companyId: string,
    actor: CashActor,
    ...
    return this.dataSource.transaction((manager) =>
      runIdempotent(
        manager,
        { companyId, userId: cashierId, operation: 'createSale', key: idempotencyKey, input, resourceType: 'sale' },
        async () => {
          ...
          // Personal por ubicación), también el administrador: si tiene que vender, se asigna.
  ...
  // Le da a la venta su consecutivo (VTA-000123) si todavía no lo tiene: se llama al cobrarla, con el
  // manager de esa transacción, y siempre al final, después de todas las comprobaciones, para que un
  // cobro que falla no gaste un número. Una venta de antes de este cambio ya lo trae y se respeta.
  // Pública porque SalePaymentService la usa.
  async assignNumber(manager: EntityManager, companyId: string, sale: Sale): Promise<void> {
    if (sale.saleNumber) return;
    sale.saleNumber = formatSaleNumber(await this.sequences.next(manager, companyId, SALE_SERIES));
  }
  ...
  // una solicitud activa. Devuelve la venta con sus totales al día. Con `idempotencyKey`, repetir la
  // misma petición (doble clic, reintento tras perder la respuesta) no duplica la línea.
  async addItem(
    companyId: string,
    actor: CashActor,
    ...
    return this.dataSource.transaction((manager) =>
      runIdempotent(
        manager,
        { companyId, userId: actor.userId, operation: 'addSaleItem', key: idempotencyKey, input, resourceType: 'sale' },
        async () => {
  ...
  // Armar la venta es del cajero de la venta: el administrador que no es ese cajero tampoco la toca (si
  // hace falta cambiarla, la anula o la cobra su cajero). Se llama con la venta ya bloqueada.
  private assertOwnDraft(sale: Sale, actor: { userId: string }): void {
    if (sale.cashierId !== actor.userId) {
      throw new ForbiddenException('Solo el cajero de la venta puede modificarla');
    }
  }
  ...
  async cancel(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      await withdrawDiscountRequests(manager, this.notifications, {
        companyId,
        saleIds: [sale.id],
        statuses: ACTIVE_DISCOUNT_REQUEST_STATUSES,
  ...
  // Trae la venta bloqueada sea cual sea su estado: para cobrar, que tiene que distinguir un reintento
  // ("ya se cobró") de una venta cancelada. Pública porque SalePaymentService la usa.
  async lockAnyStatus(manager: EntityManager, companyId: string, id: string): Promise<Sale> {
    const sale = await manager.getRepository(Sale).findOne({
      where: { id, companyId },
      lock: { mode: 'pessimistic_write' },
    });
